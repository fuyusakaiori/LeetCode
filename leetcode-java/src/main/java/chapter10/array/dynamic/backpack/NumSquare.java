package chapter10.array.dynamic.backpack;
...
public class NumSquare {
     ...
     * <h3>动态规划</h3>
     */
    public static int numSquares(int number){
     ...
     * <h3>递归</h3>
     */
    public int numSquares(int number, int[] dp){
        if(number == 0)
            return 0;
        if(dp[number] != 0)
            return dp[number];
        ...
        for(int idx = 1; idx * idx <= number;idx++){
            minCnt = Math.min(numSquares(number - idx * idx, dp) + 1, minCnt);
        }
